        ...
        viewModel.effects.collectLatest { effect ->
            when (effect) {
                ...
                is UserListEffect.ScrollToUser -> {
                    val index = state.users.indexOfFirst { it.id == effect.userId }
                    if (index >= 0) {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                ...
                actions = {
                    IconButton(
                        onClick = { viewModel.onIntent(UserListIntent.RefreshUsers) },
...
@Composable
private fun UserListContent(state: UserListState, listState: LazyListState, timeProvider: TimeProvider, selectedUser: User?, onUserClick: (User) -> Unit, onUserLongClick: (User) -> Unit, onRetry: () -> Unit, bottomPadding: Dp = 88.dp) {
    when {
        state.isLoading && state.users.isEmpty() -> ShimmerUserList()
        state.error != null && state.users.isEmpty() -> ErrorScreen(error = state.error!!, onRetry = onRetry)
        else -> UserList(users = state.users, listState = listState, timeProvider = timeProvider, selectedUser = selectedUser, onUserClick = onUserClick, onUserLongClick = onUserLongClick, bottomPadding = bottomPadding)
    }
}
...
private fun SingleColumnLayout(state: UserListState, listState: LazyListState, timeProvider: TimeProvider, onIntent: (UserListIntent) -> Unit, modifier: Modifier = Modifier) {
    Column(modifier = modifier.fillMaxSize()) {
        ...
        Box(modifier = Modifier.weight(1f)) {
            UserListContent(state = state, listState = listState, timeProvider = timeProvider, selectedUser = null, onUserClick = {}, onUserLongClick = { onIntent(
                UserListIntent.RequestDelete(it)) }, onRetry = { onIntent(UserListIntent.LoadUsers) })
        }
    }
}
    ...
    Column(modifier = modifier.fillMaxSize()) {
        ...
        Row(modifier = Modifier.weight(1f)) {
            Box(modifier = Modifier.weight(0.4f)) {
                UserListContent(state = state, listState = listState, timeProvider = timeProvider, selectedUser = state.selectedUser, onUserClick = { onIntent(
                    UserListIntent.SelectUser(it)) }, onUserLongClick = { onIntent(UserListIntent.RequestDelete(it)) }, onRetry = { onIntent(
                    UserListIntent.LoadUsers) }, bottomPadding = 8.dp)
            }
            VerticalDivider()
...
@Composable
private fun UserList(users: List<User>, listState: LazyListState, timeProvider: TimeProvider, selectedUser: User?, onUserClick: (User) -> Unit, onUserLongClick: (User) -> Unit, bottomPadding: Dp = 88.dp) {
